...
/* SE_GENI_CLK_SEL selects which DFS bank feeds the SE's serial clock. */
#define CLK_SEL_MSK			GENMASK_32(2, 0)

/*
 * Upper bound on DFS-selectable rows copied out of a SE clock's frequency
 * plan by qup_i2c_resolve_dfs(). The lemans plans expose 8; the headroom
 * absorbs a longer plan without silently ignoring the tail.
 */
#define QUP_I2C_MAX_DFS_ROWS		16u
...
/* True if this source rate has an SCL counter table above. */
static bool qup_i2c_rate_has_table(unsigned long src_hz)
{
	return src_hz == 19200000 || src_hz == 32000000;
}

/*
 * Resolve which DFS bank to point SE_GENI_CLK_SEL at, and the source rate
 * that bank actually delivers.
 *
 * This SE takes its serial clock through hardware DFS (qup_i2c_clk_setup()
 * calls qcom_clk_enable_dfs()), so the rate is chosen by writing a DFS index
 * to SE_GENI_CLK_SEL -- not by clk_set_rate(), and not by the platform cfg
 * value. The DFS index for a given rate is target-specific: on lemans 19.2 MHz
 * is index 1 and index 0 is 14.7456 MHz, while other plans put 19.2 MHz at
 * index 0. Hardcoding either one silently runs SCL at the wrong rate, because
 * the counter table is picked from the rate we *think* we selected.
 *
 * Only two source rates have counter tables, so search the clock's
 * DFS-selectable rows for one of those, preferring the platform cfg rate when
 * it is available. Mirrors geni_spi_resolve_clk() in qcom_geni_spi.c.
 */
static TEE_Result qup_i2c_resolve_dfs(struct qup_i2c_data *qi,
				       uint8_t *dfs_idx, unsigned long *src_hz)
{
	unsigned long rates[QUP_I2C_MAX_DFS_ROWS] = { };
	uint8_t indices[QUP_I2C_MAX_DFS_ROWS] = { };
	size_t count = 0;
	size_t i = 0;
	TEE_Result res = TEE_SUCCESS;

	res = qcom_clk_get_dfs_rates_array(qi->se_clk, 0, NULL, NULL, &count);
	if (res) {
		EMSG("QUP I2C %u: cannot enumerate DFS rates for %s: %#" PRIx32,
		     qi->id, qi->se_clock_name, res);
		return res;
	}
	if (!count) {
		EMSG("QUP I2C %u: clock %s exposes no DFS-selectable rates",
		     qi->id, qi->se_clock_name);
		return TEE_ERROR_ITEM_NOT_FOUND;
	}

	if (count > QUP_I2C_MAX_DFS_ROWS) {
		EMSG("QUP I2C %u: clock has %zu DFS rows, only %u inspected -- raise QUP_I2C_MAX_DFS_ROWS",
		     qi->id, count, QUP_I2C_MAX_DFS_ROWS);
		count = QUP_I2C_MAX_DFS_ROWS;
	}

	res = qcom_clk_get_dfs_rates_array(qi->se_clk, 0, rates, indices,
					   &count);
	if (res)
		return res;

	/* Preferred: the rate the platform cfg declares, if it is selectable. */
	for (i = 0; i < count; i++) {
		if (rates[i] != qi->clk_hz)
			continue;
		if (!qup_i2c_rate_has_table(rates[i]))
			break;
		*dfs_idx = indices[i];
		*src_hz = rates[i];
		return TEE_SUCCESS;
	...
	/* Otherwise any DFS rate we have counters for. */
	for (i = 0; i < count; i++) {
		if (!qup_i2c_rate_has_table(rates[i]))
			continue;
		*dfs_idx = indices[i];
		*src_hz = rates[i];
		IMSG("QUP I2C %u: cfg rate %u Hz not DFS-selectable, using %lu Hz (dfs_idx %u)",
		     qi->id, qi->clk_hz, *src_hz, *dfs_idx);
		return TEE_SUCCESS;
	}

	EMSG("QUP I2C %u: no DFS-selectable rate has an SCL counter table (need 19.2 or 32 MHz)",
	     qi->id);
...
/*
 * Picks the SCL-timing row for qi->speed_hz, given the source rate the DFS
 * bank selected by qup_i2c_resolve_dfs() actually delivers.
 *
 * The counter values are only valid for that rate -- exactly as mainline does
 * (geni_i2c_clk_map_idx() keys off clk_get_rate(gi2c->se.clk), not off a config
 * value). src_hz is passed in rather than read from qi->clk_hz: the cfg value
 * is only a request, and if the DFS bank delivers something else, using the cfg
 * number here mis-times SCL by exactly that ratio.
 */
static const struct qup_i2c_clk_fld *
qup_i2c_find_clk_fld(struct qup_i2c_data *qi, unsigned long src_hz)
{
	...
	if (src_hz != qi->clk_hz)
		DMSG("QUP I2C %u: SE clock is %lu Hz via DFS, platform cfg says %u Hz -- timing for %lu Hz",
...
static TEE_Result qup_i2c_configure_bus(struct qup_i2c_data *qi)
{
	...
	/*
	 * Resolve the DFS bank first: it determines the source rate, and the
	 * source rate determines which counter table is correct. Doing this in
	 * the other order is what makes SCL come out at the wrong frequency.
	 */
	res = qup_i2c_resolve_dfs(qi, &dfs_idx, &src_hz);
	if (res)
		return res;

	fld = qup_i2c_find_clk_fld(qi, src_hz);
	if (!fld) {
		/*
	...
	/*
	 * Point the SE at the DFS bank that delivers src_hz. Writing a fixed 0
	 * here is wrong on lemans: bank 0 is 14.7456 MHz, so the 19.2 MHz
	 * counters below would run 19.2/14.7456 = 1.30x slow (a "100 kHz" bus
	 * measures ~81 kHz). The index must come from the clock's plan.
	 */
	io_write32(qi->base + SE_GENI_CLK_SEL, dfs_idx & CLK_SEL_MSK);
	...
	DMSG("QUP I2C %u: speed=%u Hz src=%lu Hz dfs_idx=%u clk_div=%u high=%u low=%u cycle=%u",
	     qi->id, qi->speed_hz, src_hz, dfs_idx, fld->clk_div,
	     fld->t_high_cnt, fld->t_low_cnt, fld->t_cycle_cnt);
				...
				uint8_t *rdat, size_t len)
{
	...
	/*
	 * Hold the lock for the whole transfer, not just this setup. The
	 * polling loop below consumes tx_buf/rx_buf and decrements
	 * tx_rem_bytes/rx_rem_bytes through the FIFO helpers, and the
	 * short-transfer check afterwards reads them, so unlocking before the
	 * poll would leave a second caller free to overwrite that state
	 * mid-transfer. Safe to span the poll now this is a mutex and not a
	 * spinlock: no interrupts are disabled for the duration, and none of
	 * the callees below take the lock themselves.
	 */
	mutex_lock(&qi->lock);
...
/*
 * Acquire the SE source clock plus the wrapper/common clocks by name, and turn
 * on hardware DFS for the SE clock.
 *
 * DFS is what makes SE_GENI_CLK_SEL meaningful: without it only input 0 carries
 * a clock and selecting any other bank stops SCL dead. With it, the source rate
 * is chosen per transfer by qup_i2c_resolve_dfs() writing that rate's plan index
 * to SE_GENI_CLK_SEL -- so clk_set_rate() is deliberately never called on this
 * clock, matching qcom_geni_spi.c. All clocks are left gated here;
...
static TEE_Result qup_i2c_clk_setup(struct qup_i2c_data *qi)
{
	...
	if (res) {
		EMSG("QUP I2C: enable DFS on %s failed: %#" PRIx32,
		     qi->se_clock_name, res);
		qi->se_clk = NULL;
		return res;
	}
