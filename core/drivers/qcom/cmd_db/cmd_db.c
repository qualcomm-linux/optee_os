...
#include "cmd_db_target_config.h"
...
static TEE_Result get_config(paddr_t *base_addr, size_t *size_out)
{
	struct cmd_db_target_config config = { };
	struct aop_cmd_db_metadata *metadata = NULL;

	config = cmd_db_get_target_config();

	if (!config.aop_metadata_addr || !config.aop_metadata_size)
		return TEE_ERROR_BAD_PARAMETERS;

	metadata = phys_to_virt(config.aop_metadata_addr, MEM_AREA_IO_NSEC,
				config.aop_metadata_size);
	if (!metadata) {
		EMSG("Failed to get VA for AOP metadata at PA 0x%lx",
		     (unsigned long)config.aop_metadata_addr);
		return TEE_ERROR_GENERIC;
	}

	*base_addr = metadata->addr;
	*size_out = metadata->size;
...
static TEE_Result cmd_db_init(void)
{
	...
	res = get_config(&pa, &size);
	if (res != TEE_SUCCESS) {
		EMSG("CMD_DB: Failed to get configuration");
		goto err_panic;
	}

	query_db.data = core_mmu_add_mapping(MEM_AREA_IO_NSEC, pa, size);
	if (!query_db.data) {
		EMSG("CMD_DB: Failed to map at PA 0x%lx", (unsigned long)pa);
		goto err_panic;
	}
			       ...
			       struct entry_header *entry)
{
	...
	for (i = 0; i < CMD_DB_MAX_SLV_ID; i++) {
		...
		for (j = 0; j < slv_info->cnt; j++) {
			...
			memcpy(entry, query_db.data->data +
			       slv_info->header_offset +
			       sizeof(struct entry_header) * j, sizeof(*entry));
				...
				uint8_t *data)
{
	...
	memcpy(data, query_db.data->data + slv_info->data_offset +
	       entry->offset, len);
	result->len = len;
...
TEE_Result cmd_db_query_addr(const char *res_id, uint32_t *addr)
{
	struct cmd_db_query_result_type result = { };
	TEE_Result res;
...
TEE_Result cmd_db_query_priority(uint32_t addr, uint8_t drv_id,
				 uint32_t *priority)
{
	struct cmd_db_query_result_type result = { };
	TEE_Result res;
