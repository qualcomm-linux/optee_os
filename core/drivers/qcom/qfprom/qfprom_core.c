...
static enum qfprom_error get_region_name(uint32_t addr,
					 ...
					 enum qfprom_region_name *region_type)
{
...
static int raw_read(uint32_t addr,
		    enum qfprom_addr_space type,
		    uint32_t *data)
{
	uint32_t ret;
	...
	if (type == QFPROM_ADDR_SPACE_RAW) {
		ret = hal_qfprom_read_raw_address_row(addr, data);
	} else {
		addr = QFPROM_RAW_TO_CORR(addr);
		ret = hal_qfprom_read_corrected_address_row(addr, data);
	}

	return ret;
}

static enum qfprom_error is_fec_enabled(enum qfprom_region_name region_type,
					bool *fec_status)
{
	struct qfprom_context *drv = qfprom_get_context();
	...
	for (i = 0; i < drv->config->num_regions; i++) {
		if (drv->config->region_data[i].region_name == region_type) {
	...
	*fec_status = (val & (1U << bit)) != 0;

	return QFPROM_NO_ERR;
}

static bool check_region_access(enum qfprom_region_name region,
				uint32_t perm_flags)
{
	struct qfprom_context *drv = qfprom_get_context();
	const struct qfprom_region_info *info = NULL;
	...
	for (i = 0; i < drv->config->num_regions; i++) {
		if (drv->config->region_data[i].region_name == region) {
			info = &drv->config->region_data[i];
			break;
		}
	}
	...
	if (perm_flags & REGION_PERM_READ) {
		...
		if (!offset) {
			...
			if (hal_qfprom_read_corrected_address(perm_addr,
							      &perm) !=
			    QFPROM_NO_ERR)
				return false;
...
{
	enum qfprom_error err;
	enum qfprom_region_name region;
	...
	err = get_region_name(addr, QFPROM_ADDR_SPACE_RAW, &region);
	if (err != QFPROM_NO_ERR)
		return err;

	if (!check_region_access(region,
				 REGION_PERM_READ | REGION_PERM_WRITE))
		return QFPROM_REGION_NOT_WRITABLE_ERR;

	if (is_fec_enabled(region, &fec_enabled) == QFPROM_NO_ERR &&
	    fec_enabled)
		return QFPROM_REGION_NOT_WRITABLE_ERR;
	...
	if (!check_region_access(region, REGION_PERM_READ))
		return QFPROM_NO_ERR;

	err = raw_read(addr, QFPROM_ADDR_SPACE_RAW, verify);
	if (err != QFPROM_NO_ERR)
		return QFPROM_NO_ERR;
			   ...
			   uint32_t *data)
{
	...
	err = get_region_name(addr, type, &region);
	...
	if (type == QFPROM_ADDR_SPACE_CORR) {
		...
		err = is_fec_enabled(region, &fec_enabled);
		if (err == QFPROM_NO_ERR && fec_enabled &&
		    hal_qfprom_is_fec_error_seen()) {
			uint16_t err_addr = 0;
				   ...
				   uint8_t *has_fec)
{
	...
	err = get_region_name(addr, type, &region_type);
	if (err == QFPROM_ADDRESS_INVALID_ERR)
		return TEE_ERROR_BAD_PARAMETERS;
	else if (err != QFPROM_NO_ERR)
		return TEE_ERROR_GENERIC;
	...
	for (i = 0; i < drv->config->num_regions; i++) {
		...
		if (region->region_name == region_type) {
			if (check_region_access(region_type, REGION_PERM_READ))
				*has_fec = (region->fec_type !=
					    QFPROM_FEC_NONE) ? 1 : 0;
			else
				*has_fec = 0;
