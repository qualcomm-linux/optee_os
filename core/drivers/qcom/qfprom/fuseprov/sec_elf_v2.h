...
#define FUSEPROV_SECDAT_VERSION_2	0x00000002

#define FUSEPROV_SECDAT_MAX_SUPPORTED_SEGMENT	32

#define FUSEPROV_SECDAT_SEGMENT_TYPE_EFUSE	0x00
#define FUSEPROV_SECDAT_SEGMENT_TYPE_ENCKEY	0x01
...
TEE_Result sec_elf_validate_header(const uint8_t *data, size_t size,
				   const struct secdat_hdr **hdr);
...
TEE_Result blow_shk(const struct fuse_entry *entries, uint32_t count);

TEE_Result fuseprov_execute(const uint8_t *data, size_t len);

void fuseprov_reset_device(void);
