			 ...
			 const struct segment_hdr **segments)
{
	...
	if (header->magic1 != FUSEPROV_SECDAT_MAGIC1) {
		EMSG("Invalid magic1: 0x%08x", header->magic1);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->magic2 != FUSEPROV_SECDAT_MAGIC2) {
		EMSG("Invalid magic2: 0x%08x", header->magic2);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->revision != FUSEPROV_SECDAT_VERSION_2) {
	...
	if (header->seg_num > FUSEPROV_SECDAT_MAX_SUPPORTED_SEGMENT) {
				   ...
				   const struct secdat_hdr **hdr)
{
	...
	if (header->magic1 != FUSEPROV_SECDAT_MAGIC1 ||
	    header->magic2 != FUSEPROV_SECDAT_MAGIC2) {
