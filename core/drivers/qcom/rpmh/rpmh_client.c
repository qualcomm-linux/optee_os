...
static bool tcs_is_idle(enum rscsw_drv_mapping drv_id, uint32_t timeout_us)
{
...
static struct client *find_client(enum rscsw_drv_mapping drv_id,
				  const char *name)
{
...
static struct client *create_client(enum rscsw_drv_mapping drv_id,
...
static bool wait_for_cmd(struct client *client, uint32_t req_id, bool all)
{
	...
	while (true) {
		if (all ? client->num_reqs_in_progress == 0 :
		    (hal_rpmh_is_tcs_idle(client->drv_id, 0, &(bool){false}) ==
		     HAL_STATUS_SUCCESS))
			break;
	...
	if (!timed_out && client->num_reqs_in_progress > 0)
		client->num_reqs_in_progress--;

	return !timed_out;
}

static TEE_Result check_aop_init(bool *ready)
{
	vaddr_t base, dict_addr, cookie_addr;
				       ...
				       const char *name)
{
	...
	if (!name || !rpmh_is_drv_id_valid(drv_id))
		return NULL;
...
TEE_Result rpmh_issue_command(struct rpmh_client *handle,
			      enum rpmh_set_enum set, bool completion,
			      ...
			      uint32_t *req_id)
{
	...
	*req_id = 0;
	...
	if (id == 0 || !wait_for_cmd(client, id, false)) {
		res = (id == 0) ? TEE_ERROR_GENERIC : TEE_ERROR_BUSY;
		goto out;
	}
