 ...
 * carry a real DFS performance-state index; rows without one have no bank the
 * SE can select. qcom_clk_get_dfs_rates_array() copies out exactly the
 * selectable (rate, dfs_index) pairs, so no filtering is needed here -- the
 * clock driver keeps its config table private. The SE's own serial divider
 * then brings the chosen source rate down to SCLK: div = source_rate/speed_hz.
 *
				       ...
				       uint8_t *dfs_idx, uint32_t *clk_div)
{
	...
	/* Probe how many DFS-selectable rows this clock has. */
	...
	/*
	 * Only ever ask for what fits: the copy-out call rejects a count past
	 * the row total. A plan with more rows than this is a config the driver
	 * was not built for, so say so rather than silently ignoring the tail.
	 */
