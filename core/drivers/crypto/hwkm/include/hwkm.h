...
struct hwkm_key_policy {
	...
	bool km_by_modem_allowed;	/* Modem may use this slot. */
	bool km_by_spu_allowed;		/* SPU may use this slot. */
	enum hwkm_algo alg_allowed;	/* Only algorithm permitted for this key. */
	bool enc_allowed;		/* Key may be used for encryption. */
	bool dec_allowed;		/* Key may be used for decryption. */
	enum hwkm_key_type key_type;	/* Functional role. */
	uint8_t kdf_depth;		/* Remaining KDF derivation hops. */
	...
	enum hwkm_key_security_lvl security_lvl;  /* SW_KEY, MANAGED_KEY, or HW_KEY. */
	enum hwkm_key_destination hw_destination; /* Hardware destination. */
	bool wrap_with_tpkey_allowed;	/* Allow wrapping under TPKEY. */
};
...
struct hwkm_bsve {
	...
	bool km_mks_en;			/* Include the mixing-key slot value in the KDF. */
	uint64_t km_fuse_region_sha_digest_en; /* Fuse regions hashed into KDF. */
};

/* Command and response structs. */

enum hwkm_op {
	HWKM_OP_NIST_KEYGEN = 0,	/* Generate a key via HW PRNG. */
	HWKM_OP_SYSTEM_KDF = 1,		/* Derive a child key from a KDK slot. */
	/* Opcode 2 is reserved. */
	HWKM_OP_KEY_WRAP_EXPORT = 3,	/* Wrap a slot under a KWK/KSK. */
	HWKM_OP_KEY_UNWRAP_IMPORT = 4,	/* Unwrap a blob into a destination slot. */
	HWKM_OP_KEY_SLOT_CLEAR = 5,	/* Clear a slot and invalidate its policy. */
	HWKM_OP_KEY_SLOT_RDWR = 6,	/* Read or write raw key material. */
	HWKM_OP_SET_TPKEY = 7,		/* Install a slot as the active TPKEY. */
};
...
static inline TEE_Result hwkm_to_tee(int rc)
{
	switch (rc) {
	case HWKM_SUCCESS:
		return TEE_SUCCESS;
