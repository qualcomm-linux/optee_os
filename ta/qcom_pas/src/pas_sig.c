				     ...
				     size_t *roots_der_len)
{
	...
	rc = mbedtls_x509_crt_verify_with_profile(leaf, &trust, NULL,
						  ...
						  &flags, NULL, NULL);
	/* No trusted time: clear expired/future flags, keep rest fatal. */
...
{
	uint8_t digest_buf[PAS_AUTH_MAX_HASH_SIZE] = { };
	...
	if (sig_len > PAS_AUTH_MAX_SIG_SIZE)
		return TEE_ERROR_SECURITY;
	...
	res = digest(hash_algo, msg, msg_len, digest_buf, &digest_buf_len);
	if (res != TEE_SUCCESS)
		return res;
